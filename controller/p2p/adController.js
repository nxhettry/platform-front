...
export async function getFilteredAdsMob(req, res) {
    ...
    if (!email || !filterStates) {
        return res.status(400).json({ message: "Email and filter states are required" });
    }
    ...
    try {
        ...
        const { isBuy, isOnline, isOffline, isNPR, isINR, isAED, isUSD } = filterStates;
...
export async function validateAsset(req, res) {
    ...
    try {
        ...
        if (currency.amount < toCheckAmount) {
            return res.status(200).json({ message: "Insufficient balance", data: false });
        }
